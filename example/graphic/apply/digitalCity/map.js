...
function onMounted(mapInstance) {
  ...
  // // 围绕旋转
  // var rotatePoint = new mars3d.thing.RotatePoint({
...
function addCityGraphics() {
  ...
  // 四面体
  var tetrahedron = new mars3d.graphic.Tetrahedron({
    position: [position[0], position[1], 180],
    style: {
      width: 20,
      height: 30,
  ...
  mars3d.Util.fetchJson({ url: "//data.mars3d.cn/file/geojson/shanghai-point.json" })
    .then((result) => {
      ...
      for (let i = 0, len = pointArr.length; i < len; i++) {
        ...
        var color = ["#ffff00", "#81d8ff", "#fff9ed"]
        var thisPoint = Cesium.Cartesian3.fromDegrees(item.point[0], item.point[1], 1)
  ...
  for (let j = 0; j < 100; ++j) {
    ...
    var startTime = random(0, 10000)
    var speed = random(1, 35)
