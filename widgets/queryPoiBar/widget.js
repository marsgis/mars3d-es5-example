...
;(function (window, mars3d) {
  ...
  class MyWidget extends es5widget.BaseWidget {
    ...
    //初始化[仅执行1次]
    create() {
      this.storageName = "mars3d_queryPoiBar"
      this.pageSize = 6
      this.allpage = 0
      this.thispage = 0
      ...
      //查询控制器
      this._queryPoi = new mars3d.query.QueryPOI({
        // city: '合肥市',
      })
    }
    //每个窗口创建完成后调用
      ...
      this._queryPoi.autoTip({
        ...
        success: (result) => {
          ...
          for (let index = 0; index < pois.length; index++) {
            ...
            inhtml += "<li><i class='fa fa-search'></i><a href=\"javascript:queryPoiBarWidget.autoSearch('" + name + "');\">" + name + "</a></li>"
          }
          if (inhtml.length > 0) {
      ...
      if (counts == 0) {
        ...
        if (this.allpage > 1) {
          _fyhtml =
            ...
            "/" +
            this.allpage +
            '页  <a href="javascript:queryPoiBarWidget.showFirstPage()">首页</a> <a href="javascript:queryPoiBarWidget.showPretPage()">&lt;</a>  <a href="javascript:queryPoiBarWidget.showNextPage()">&gt;</a> </div>'
        } else {
          _fyhtml = ""
        }
    ...
    showHistoryList() {
      ...
      localforage.getItem(this.storageName).then((laststorage) => {
        ...
        for (let index = this.arrHistory.length - 1; index >= 0; index--) {
          ...
          inhtml += "<li><a href=\"javascript:queryPoiBarWidget.autoSearch('" + item + "');\">" + item + "</a></li>"
        }
  ...
  //注册到widget管理器中。
  window.queryPoiBarWidget = es5widget.bindClass(MyWidget)
